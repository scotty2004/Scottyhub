...
async function initDB() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS cpa_conversions (
      ...
      cops_credited INTEGER NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS spin_history (
      ...
      spin_date TEXT NOT NULL DEFAULT (NOW()::date::text),
      created_at TEXT DEFAULT (NOW()::text),
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS bot_purchases_one_buy_per_listing
      ON bot_purchases (listing_id, buyer_id) WHERE type = 'buy';
  ...
  await pool.query(`
    DELETE FROM bot_purchases a USING bot_purchases b
    WHERE a.type='buy' AND b.type='buy' AND a.listing_id=b.listing_id AND a.buyer_id=b.buyer_id AND a.id > b.id
  `);
  await pool.query(`
    CREATE UNIQUE INDEX IF NOT EXISTS bot_purchases_one_buy_per_listing
      ON bot_purchases (listing_id, buyer_id) WHERE type = 'buy'
  ...
  await pool.query(`ALTER TABLE news ADD COLUMN IF NOT EXISTS content_type TEXT DEFAULT 'news'`);
  await pool.query(`ALTER TABLE news ADD COLUMN IF NOT EXISTS media_url TEXT DEFAULT ''`);
  await pool.query(`ALTER TABLE news ADD COLUMN IF NOT EXISTS media_type TEXT DEFAULT ''`);
  await pool.query(`ALTER TABLE spin_history ADD COLUMN IF NOT EXISTS spin_date TEXT`);
  await pool.query(`UPDATE spin_history SET spin_date = substring(created_at from 1 for 10) WHERE spin_date IS NULL`);
  await pool.query(`
    DELETE FROM spin_history a USING spin_history b
    WHERE a.user_id = b.user_id AND a.spin_date = b.spin_date AND a.id > b.id
  `);
  await pool.query(`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'spin_history_user_id_spin_date_key'
      ) THEN
        ALTER TABLE spin_history ADD CONSTRAINT spin_history_user_id_spin_date_key UNIQUE (user_id, spin_date);
      END IF;
    END $$;
  `);
